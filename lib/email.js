...
// curl -X POST "https://getdealdrop-tracker.vercel.app/api/cron/check-prices" -H "Authorization: Bearer e1e8f56b90ab13634d0ddc242a65887c789e1333a23fd09baebc3c63109cd79a"
...
export async function sendPriceDropAlert(
  ...
  try {
    ...
    const mailOptions = {
      from: `"DealDrop Alerts" <${process.env.EMAIL_USER}>`,
      to: userEmail,
      subject: `🔥 Price Drop Alert: ${product.name}`,
      html: `
        <h2>🎉 Price Drop Alert!</h2>
        <p><strong>${product.name}</strong></p>
        <p>Old Price: ${product.currency} ${oldPrice}</p>
        <p>New Price: ${product.currency} ${newPrice}</p>
        <p>You Save: ${product.currency} ${priceDrop} (${percentageDrop}%)</p>
